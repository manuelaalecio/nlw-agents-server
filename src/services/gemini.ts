...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
  const response = await gemini.models.generateContent({
    ...
    contents: [
      {
        text: 'Transcribe the audio to Brazilian portuguese. Be precise and natural in your transcription. Keep punctuation and divide text into paragraphs when appropriate.',
      },
      {
        inlineData: {
          mimeType,
...
export async function generateAnswer(
  ...
  - Cite trechos do contexto de apropriado;
